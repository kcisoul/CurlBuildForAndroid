...
RUN cp -r openssl-1.0.2d /Android/output/openssl
...
ENV CPPFLAGS $CPPFLAGS $CFLAGS -L/Android/openssl-1.0.2d/include
ENV LDFLAGS -L${TOOLCHAIN}/include -march=$ARCH -Wl,--fix-cortex-a8 -L/Android/openssl-1.0.2d


RUN curl -O http://curl.haxx.se/download/curl-7.45.0.tar.gz && \
	tar -xzf curl-7.45.0.tar.gz
RUN cd curl-7.45.0 && ./configure --host=arm-linux-androideabi --disable-shared --enable-static --disable-dependency-tracking --with-zlib=/Android/zlib --with-ssl=/Android/openssl-1.0.2d --without-ca-bundle --without-ca-path --enable-ipv6 --enable-http --enable-ftp --disable-file --disable-ldap --disable-ldaps --disable-rtsp --disable-proxy --disable-dict --disable-telnet --disable-tftp --disable-pop3 --disable-imap --disable-smtp --disable-gopher --disable-sspi --disable-manual --target=arm-linux-androideabi --build=x86_64-unknown-linux-gnu --prefix=/opt/curlssl || cat config.log
...
RUN cp -r curl-7.45.0 /Android/output/curl
